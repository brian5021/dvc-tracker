...
export async function getDatabaseStats() {
  return databaseRepository.getStats()
}
...
// Add a new action to fix negative banked points
export async function fixNegativeBankedPoints() {
  return pointService.fixNegativeBankedPoints()
}

// Add a new action to clean up duplicate transfers
export async function cleanupDuplicateTransfers() {
  try {
    ...
    for (const transfer of transfers) {
      // Create a unique key for each transfer
      const key = `${transfer.fromMember}-${transfer.toMember}-${transfer.points}-${transfer.transferDate}`

      if (transferIds.has(key)) {
        duplicates.push(transfer.id)
      } else {
        transferIds.add(key)
      }
    }
    ...
    for (const id of duplicates) {
      ...
      if (transfer && transfer.status === "Active") {
        // Mark as returned first to update points
        await transferService.updateTransferStatus(id, "Returned")
        deletedCount++
      }
    }
    ...
    return {
      success: true,
      message: `Cleaned up ${deletedCount} duplicate transfers`,
      deletedCount,
    }
  } catch (error) {
    console.error("Error cleaning up duplicate transfers:", error)
    return {
      success: false,
      message: "Failed to clean up duplicate transfers",
